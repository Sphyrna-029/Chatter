...
- A **profile** theme is a different thing from the app theme and shares none of its machinery. The app theme is the viewer's choice about their own client and covers every surface; a profile theme belongs to the person, is set by them, and reaches exactly two places — their profile modal and their row in the member list. It is three fields on the user record (`profile_color`, `profile_fade`, `profile_fade_direction`) that ride along with presence like `name_font_url` does, so a member row already has it without a second fetch. `lib/profileTheme.ts` is the single definition of what those three mean: the colour holds for `100 - fade` percent and gives out over the rest, so fade 0 is an even wash and fade 100 fades across the whole surface, and the alpha it is laid on at is per-surface (`PROFILE_WASH_ALPHA`) because a row has to stay a list before it is a colour. Both ends fade to the *same colour at zero alpha* rather than to `transparent`, which is transparent black and greys the middle of the gradient
- The colour reaches a CSS gradient on every client that draws the person, so it is `#rrggbb` on both sides (`helpers::valid_profile_color`, `readProfileTheme`) and the direction is a closed set of four; the fade is clamped rather than refused, since the slider cannot produce an out-of-range one. The profile banner is *masked* rather than covered by a fade-to-background overlay — an opaque overlay paints over the wash behind it and leaves a seam where the banner ends
