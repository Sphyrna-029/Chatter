...
- `referenced_at` on an upload records that something keeps it — set by `send_message`, `edit_message`, `send_thread_message` and `set_profile`. Nothing reads it yet; it is what a sweep for files nothing ever referenced will skip, so the common paths are already claimed by the time that lands
