...
- Mute and deafen belong to the person, not the channel: `joinVoice` preserves both when it is entered while already in a call — a rejoin or a channel switch — silences the fresh track before publishing, and re-announces `voice_mute`/`voice_deafen`, since the server treats every join as an unmuted arrival
