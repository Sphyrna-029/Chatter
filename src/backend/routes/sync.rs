...
pub(crate) async fn sync(
    ...
    // Rooms have nothing to do with one another, so they are built at the same
    // time rather than one after the next. Each one is a dozen or so round
    // trips that no other room waits on, and an account in ten of them used to
    // pay all of that end to end.
    let mut pending = Vec::with_capacity(my_rooms.len());
    for (room_id, members) in &my_rooms {
        pending.push(build_joined_room(&state, &user_id, room_id, members));
    }
    let built: Vec<Option<(String, Value)>> = futures_util::stream::iter(pending)
        .buffer_unordered(ROOMS_IN_FLIGHT)
        .collect()
        .await;

    let mut joined_rooms_data = serde_json::Map::new();
    for (room_id, room) in built.into_iter().flatten() {
        joined_rooms_data.insert(room_id, room);
    }
...
/// How many rooms `sync` builds at once. Enough to hide the per-room latency
/// of an account in a handful of rooms, and far short of the connection pool,
/// so a sync cannot starve every other request of a connection.
const ROOMS_IN_FLIGHT: usize = 8;

/// One room's entry in a sync response: its state, its membership, and the tail
/// of its timeline. `None` when the room has gone from the database but is
/// still in the membership cache.
