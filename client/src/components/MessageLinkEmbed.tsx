...
import { Hash, MessageSquareQuote, Paperclip } from "lucide-react";
 ...
 * is the point of the feature rather than an error path: the same message,
 * posted in a channel half the room cannot read, shows them a link and no
 * content. Nothing is decided here — the server refuses to resolve the link
...
export function MessageLinkEmbed({ eventId }: { eventId: string }) {
  ...
  return (
    <button
      ...
      className="mt-1 flex w-full max-w-[min(520px,100%)] flex-col gap-1 rounded-md border border-border bg-secondary/40 p-2.5 text-left transition-colors hover:bg-secondary/70 cursor-pointer"
    >
