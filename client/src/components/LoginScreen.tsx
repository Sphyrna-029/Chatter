...
export function LoginScreen() {
  ...
  // Handle Steam OAuth redirect params on initial load
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const steamAccess = params.get("steam_access");
    const steamRefresh = params.get("steam_refresh");
    const steamUserId = params.get("steam_user_id");
    const steamIsAdmin = params.get("steam_is_admin");
    const steamTotp = params.get("steam_totp");
    const steamError = params.get("steam_error");
    const steamLinked = params.get("steam_linked");

    if (steamLinked === "true") {
      window.history.replaceState({}, "", "/");
      // Just clean up the URL; the user is already logged in
      return;
    }

    if (steamAccess && steamRefresh && steamUserId) {
      window.history.replaceState({}, "", "/");
      setAccessToken(steamAccess);
      setRefreshToken(steamRefresh);
      setIsAdmin(steamIsAdmin === "true");
      setTotpVerified(steamTotp === "true");
      dispatch({ type: "SET_IS_ADMIN", payload: steamIsAdmin === "true" });
      dispatch({ type: "SET_TOTP_VERIFIED", payload: steamTotp === "true" });
      dispatch({ type: "LOGIN", payload: { accessToken: steamAccess, userId: steamUserId } });
    } else if (steamError) {
      window.history.replaceState({}, "", "/");
      const messages: Record<string, string> = {
        verification_failed: "Steam verification failed. Please try again.",
        no_account: "No account linked to this Steam ID. Please log in and link your account first.",
        ...
        invalid_mode: "Invalid Steam login mode.",
      };
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="flex justify-center pt-1">
              <button
                ...
                style={{ background: "#171a21", color: "#c6d4df", border: "1px solid rgba(198,212,223,0.2)" }}
              >
